...
// Button Pin Setup
const int l_up_button = 11; //Left player up button pin
const int l_down_button = 12; //Left player down button pin
...
void loop() {
  ...
  if (digitalRead(l_down_button) == LOW && l_pos < (y_pixels - paddle_height)) {
    l_pos += 1;
  }
  else if (digitalRead(l_up_button) == LOW && l_pos > 0) {
    l_pos -= 1;
  }
